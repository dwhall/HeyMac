...
class GpioAhsm(pq.Ahsm):
    ...
    def initial(me, event):
        ...
        sig_dio0 = pq.Signal.register(dio0.sig_name)
        sig_dio1 = pq.Signal.register(dio1.sig_name)
        sig_dio2 = pq.Signal.register(dio2.sig_name)
        sig_pps = pq.Signal.register(pps.sig_name)
    ...
    def initializing(me, event):
        ...
        if sig == pq.Signal.ENTRY:
            ...
            GPIO.add_event_detect(dio0.pin, edge=GPIO.RISING, callback=lambda x: gpio_input_handler(sig_dio0))
            GPIO.add_event_detect(dio1.pin, edge=GPIO.RISING, callback=lambda x: gpio_input_handler(sig_dio1))
            GPIO.add_event_detect(dio2.pin, edge=GPIO.RISING, callback=lambda x: gpio_input_handler(sig_dio2))
            GPIO.add_event_detect(pps.pin, edge=GPIO.RISING, callback=lambda x: gpio_input_handler(sig_pps))
