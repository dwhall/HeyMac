...
class TestHeyMacFrame(unittest.TestCase):
    ...
    def test_min(self,):
        ...
        f.fctl_type = heymac.FCTL_TYPE_MIN
        b = bytes(f)
        self.assertEqual(b, b"\x00")
        # Unpack
        f = heymac.HeyMacFrame(b)
        self.assertEqual(f.fctl_type, heymac.FCTL_TYPE_MIN)
    ...
    def test_mac(self,):
        # Pack
        f = heymac.HeyMacFrame()
        f.fctl_type = heymac.FCTL_TYPE_MAC
        b = bytes(f)
        self.assertEqual(b, b"\x40\x10")
        # Unpack
        f = heymac.HeyMacFrame(b)
        self.assertEqual(f.fctl_type, heymac.FCTL_TYPE_MAC)
        self.assertEqual(f.fctl_l, 0)
        self.assertEqual(f.fctl_r, 0)
        self.assertEqual(f.fctl_n, 0)
        self.assertEqual(f.fctl_d, 0)
    ...
    def test_ext_using_fctl(self,):
        # Pack
        f = heymac.HeyMacFrame()
        f.fctl_type = heymac.FCTL_TYPE_EXT
        b = bytes(f)
        self.assertEqual(b, b"\xC0\x10\x00")
        # Unpack
        f = heymac.HeyMacFrame(b)
        self.assertEqual(f.fctl_type, heymac.FCTL_TYPE_EXT)
        self.assertEqual(f.fctl_l, 0)
        self.assertEqual(f.fctl_r, 0)
        self.assertEqual(f.fctl_n, 0)
        self.assertEqual(f.fctl_d, 0)
