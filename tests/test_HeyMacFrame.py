...
class TestHeyMacFrame(unittest.TestCase):
    ...
    def test_min(self,):
        ...
        f.fctl_type = mac_frame.FCTL_TYPE_MIN
        b = bytes(f)
        self.assertEqual(b, b"\x00")
        # Unpack
        f = mac_frame.HeyMacFrame(b)
        self.assertEqual(f.fctl_type, mac_frame.FCTL_TYPE_MIN)
    ...
    def test_mac(self,):
        # Pack
        f = mac_frame.HeyMacFrame()
        f.fctl_type = mac_frame.FCTL_TYPE_MAC
        f.raddr = b"\x00\x00"
        b = bytes(f)
    ...
    def test_net(self,):
        # Pack
        f = mac_frame.HeyMacFrame()
        f.fctl_type = mac_frame.FCTL_TYPE_NET
        f.raddr = b"\x00\x00"
        b = bytes(f)
        self.assertEqual(b, b"\x80\x10\x00\x00")
        # Unpack
        f = mac_frame.HeyMacFrame(b)
        self.assertEqual(f.fctl_type, mac_frame.FCTL_TYPE_NET)
        self.assertEqual(f.ver, 1)
        self.assertEqual(f.seq, 0)
        self.assertEqual(f.raddr, b"\x00\x00")
        self.assertEqual(f.daddr, b"")
    ...
    def test_ext_using_fctl(self,):
        # Pack
        f = mac_frame.HeyMacFrame()
        f.fctl_type = mac_frame.FCTL_TYPE_EXT
        f.raddr = b"\x00\x00"
        b = bytes(f)
        self.assertEqual(b, b"\xC0\x10\x00\x00\x00")
        # Unpack
        f = mac_frame.HeyMacFrame(b)
        self.assertEqual(f.fctl_type, mac_frame.FCTL_TYPE_EXT)
        self.assertEqual(f.ver, 1)
        self.assertEqual(f.seq, 0)
