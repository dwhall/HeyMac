...
from heymac.lnk.heymac_frame import HeymacFramePidIdent
...
class TestAll(unittest.TestCase):
    ...
    def test_mac_min(self):
        fmac = HeymacFrame(HeymacFramePidType.TDMA)
        b = bytes(fmac)
        self.assertEqual(b, b"\xe0\x00")

        fmac = HeymacFrame.parse(b)
        self.assertEqual(fmac.pid, HeymacFramePidIdent.HEYMAC)
    ...
    def test_mac_net_udp_min(self):
        fmac = HeymacFrame(HeymacFramePidType.CSMA)
        fnet = APv6Packet()
        ...
        fmac = HeymacFrame.parse(b)
        self.assertEqual(fmac.pid, HeymacFramePidIdent.HEYMAC | HeymacFramePidType.CSMA)
        self.assertFalse(fmac.is_extended())
        self.assertFalse(fmac.is_long_addrs())
        self.assertFalse(fmac.is_netid_present())
        self.assertFalse(fmac.is_daddr_present())
