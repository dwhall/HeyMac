...
class TestHeyMacCmds(unittest.TestCase):
    ...
    def test_ebcn(self,):
        ...
        ebcn.ntwks = []
        b = bytes(ebcn)
        self.assertEqual(b, b"\x02\xd7\x02\x03\x04\x00\x00\x00\x2a\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                b"KC4KSU-123\x00\x00\x00")
        # Unpack
        f = heymac.HeyMacCmdEbcn(b)
        self.assertEqual(f.cmd, heymac.HeyMacCmdId.EBCN)
        self.assertEqual(f.dscpln, 2)
        self.assertEqual(f.caps, 3)
        self.assertEqual(f.status, 4)
        self.assertEqual(f.asn, 42)
